...
module.exports = exports = class Server extends EventEmitter {
  ...
  /** @returns {string} The current Bluetooth adapter state. */
  get state() {
    return this._state
  }
  ...
  startAdvertising(opts = {}) {
    const uuids = opts.serviceUUIDs ? opts.serviceUUIDs.map((s) => binding.createCBUUID(s)) : null
   ...
   *
   * For read requests, set `data` to the value the central should receive
   * (sliced from `request.offset` if needed).
   *
   * For write requests, respond
   * to the first request in the array; all requests succeed or fail together.
   *
  ...
  respondToRequest(request, result, data) {
    binding.serverRespondToRequest(this._handle, request.handle, result, data || null)
  }
   ...
   * @returns {boolean} True if the value was queued for sending
   */
  ...
  /**
   * @param {object} [opts]
   * @param {boolean} [opts.encrypted=false] - Whether to require encryption on the channel.
   */
  publishChannel(opts = {}) {
  ...
  /**
   * @param {number} psm - The PSM returned by the `channelPublish` event.
   */
