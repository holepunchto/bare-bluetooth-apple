...
module.exports = exports = class PeripheralManager extends EventEmitter {
  constructor() {
    super()
  ...
  startAdvertising({ name, serviceUUIDs, serviceData } = {}) {
    const uuids = serviceUUIDs ? serviceUUIDs.map((s) => binding.createCBUUID(s)) : undefined
    binding.serverStartAdvertising(this._handle, name, uuids, serviceData)
  }
  ...
  respondToRequest(request, result, data) {
    binding.serverRespondToRequest(this._handle, request._handle, result, data || undefined)
  }
  ...
  destroy() {
    binding.serverStopAdvertising(this._handle)
    binding.serverRemoveAllServices(this._handle)
  ...
  [Symbol.for('bare.inspect')]() {
    return {
      __proto__: { constructor: PeripheralManager },
      state: this._state
    }
  }
  ...
  _onadvertiseerror(error) {
    this.emit('advertiseError', 0, error)
  }
}
