...
- (void)peripheral:(CBPeripheral *)p
  ...
  if (characteristic.isNotifying) {
    ...
    event->uuid = characteristic.UUID.UUIDString.UTF8String;
    if (error) event->error = error.localizedDescription.UTF8String;
    ...
    if (error) {
      event->data = NULL;
      event->data_len = 0;
      event->error = error.localizedDescription.UTF8String;
    } else {
      NSData *value = characteristic.value;
...
template <typename T>
static void
bare_bluetooth_apple__on_finalize(js_env_t *, T *ctx) {
  CFBridgingRelease((__bridge CFTypeRef) ctx->handle);
  delete ctx;
}

static void
bare_bluetooth_apple__on_cleanup_close(uv_handle_t *handle) {
...
bare_bluetooth_apple_peripheral__on_read(
  ...
  js_call_function(env, function, js_receiver_t(receiver), js_object_t(characteristic), uuid, js_object_t(data), error);
...
bare_bluetooth_apple_peripheral__on_write(
  ...
  js_call_function(env, function, js_receiver_t(receiver), js_object_t(characteristic), uuid, error);
...
bare_bluetooth_apple_peripheral__on_notify_state(
  ...
  js_call_function(env, function, js_receiver_t(receiver), js_object_t(characteristic), uuid, is_notifying, error);
...
bare_bluetooth_apple_peripheral__on_channel_open(
  ...
  js_call_function(env, function, js_receiver_t(receiver), js_object_t(channel), error);
...
bare_bluetooth_apple_peripheral__on_cleanup(uv_async_t *async) {
  ...
  err = js_finish_deferred_teardown_callback(wrapper->teardown);
  assert(err == 0);

  uv_close(reinterpret_cast<uv_handle_t *>(async), bare_bluetooth_apple__on_cleanup_close);
}

static void
bare_bluetooth_apple_peripheral__teardown(BareBluetoothApplePeripheral *wrapper) {
...
static void
bare_bluetooth_apple_peripheral__on_teardown(js_deferred_teardown_t *handle, void *data) {
...
bare_bluetooth_apple_peripheral__wrap(js_env_t *env, dispatch_queue_t queue, const void *peripheral) {
  ...
  wrapper->peripheral = (__bridge CBPeripheral *) peripheral;
  wrapper->queue = queue;
  dispatch_retain(wrapper->queue);

  js_value_t *result;
  int err = js_create_external(env, (__bridge void *) wrapper, bare_bluetooth_apple__on_bridged_release, NULL, &result);
  assert(err == 0);
...
bare_bluetooth_apple_peripheral_init(
  ...
  @autoreleasepool {
    ...
    BareBluetoothApplePeripheral *handle;
    err = js_get_value(env, peripheral_handle, handle);
    assert(err == 0);

    assert(handle->ctx == NULL);
    ...
    err = js_add_deferred_teardown_callback(env, bare_bluetooth_apple_peripheral__on_teardown, (__bridge void *) handle, &handle->teardown);
    assert(err == 0);
    ...
    err = js_create_threadsafe_function<
      bare_bluetooth_apple_peripheral__on_services_discover,
      bare_bluetooth_apple__on_finalize<bare_bluetooth_apple_peripheral_t>,
...
bare_bluetooth_apple_peripheral_destroy(
  ...
  @autoreleasepool {
    ...
    assert(wrapper->cleanup_async != NULL);

    bare_bluetooth_apple_peripheral__teardown(wrapper);
  }
}
...
bare_bluetooth_apple_peripheral_discover_services(
  ...
  @autoreleasepool {
    ...
    assert(wrapper->peripheral.delegate == wrapper);
...
- (void)peripheralManager:(CBPeripheralManager *)peripheral
  ...
  event->service = CFBridgingRetain(service);
  event->uuid = service.UUID.UUIDString.UTF8String;
  if (error) event->error = error.localizedDescription.UTF8String;
...
- (void)peripheralManager:(CBPeripheralManager *)peripheral
  ...
  event->characteristic_uuid = characteristic.UUID.UUIDString.UTF8String;
...
using bare_bluetooth_apple_server__on_add_service_fn = js_function_t<void, js_receiver_t, js_object_t, std::string, std::optional<std::string>>;
...
using bare_bluetooth_apple_server__on_channel_publish_fn = js_function_t<void, js_receiver_t, uint32_t, std::optional<std::string>>;
using bare_bluetooth_apple_server__on_channel_open_fn = js_function_t<void, js_receiver_t, js_object_t, std::optional<std::string>>;
using bare_bluetooth_apple_server__on_advertise_error_fn = js_function_t<void, js_receiver_t, std::string>;
...
bare_bluetooth_apple_server__on_add_service(
  ...
  js_call_function(env, function, js_receiver_t(receiver), js_object_t(service), uuid, error);
...
bare_bluetooth_apple_server__on_channel_publish(
  ...
  js_call_function(env, function, js_receiver_t(receiver), psm, error);
...
bare_bluetooth_apple_server__on_advertise_error(
  ...
  std::string error = std::move(event->error);

  delete event;

  js_call_function(env, function, js_receiver_t(receiver), error);
...
bare_bluetooth_apple_server__on_cleanup(uv_async_t *async) {
  ...
  err = js_finish_deferred_teardown_callback(server->teardown);
  assert(err == 0);
...
static void
bare_bluetooth_apple_server__teardown(BareBluetoothAppleServer *server) {
  if (server->destroyed) return;
  ...
  dispatch_async(server->queue, ^{
    uv_async_send(server->cleanup_async);
  });
}

static void
bare_bluetooth_apple_server__on_teardown(js_deferred_teardown_t *handle, void *data) {
  bare_bluetooth_apple_server__teardown((__bridge BareBluetoothAppleServer *) data);
}
...
bare_bluetooth_apple_server_init(
  ...
  @autoreleasepool {
    ...
    err = js_add_deferred_teardown_callback(env, bare_bluetooth_apple_server__on_teardown, (__bridge void *) handle, &handle->teardown);
    assert(err == 0);

    auto *state_change_ctx = new bare_bluetooth_apple_server_t{(__bridge BareBluetoothAppleServer *) CFBridgingRetain(handle)};
    err = js_create_threadsafe_function<
      bare_bluetooth_apple_server__on_state_change,
      bare_bluetooth_apple__on_finalize<bare_bluetooth_apple_server_t>,
    ...
    handle->cleanup_async = new uv_async_t;
    err = uv_async_init(loop, handle->cleanup_async, bare_bluetooth_apple_server__on_cleanup);
    assert(err == 0);
...
bare_bluetooth_apple_server_destroy(
  ...
  @autoreleasepool {
    ...
    bare_bluetooth_apple_server__teardown(server);
  }
}
...
- (void)centralManager:(CBCentralManager *)central
  ...
  event->id = idString.UTF8String;

  NSString *peripheralName = peripheral.name;
  if (peripheralName) event->name = peripheralName.UTF8String;
...
- (void)centralManager:(CBCentralManager *)central
  ...
  event->peripheral = CFBridgingRetain(peripheral);
  event->id = peripheral.identifier.UUIDString.UTF8String;
...
- (void)centralManager:(CBCentralManager *)central
  ...
  event->id = peripheral.identifier.UUIDString.UTF8String;
  event->error = error ? error.localizedDescription.UTF8String : "Unknown connection failure";
...
using bare_bluetooth_apple_central__on_discover_fn = js_function_t<void, js_receiver_t, js_object_t, std::string, std::optional<std::string>, int32_t, js_object_t>;
using bare_bluetooth_apple_central__on_connect_fn = js_function_t<void, js_receiver_t, js_object_t, std::string>;
using bare_bluetooth_apple_central__on_disconnect_fn = js_function_t<void, js_receiver_t, std::string, std::optional<std::string>>;
...
bare_bluetooth_apple_central__on_discover(
  ...
  js_value_t *peripheral_ext = bare_bluetooth_apple_peripheral__wrap(env, central->queue, event->peripheral);

  std::string id(event->id);

  std::optional<std::string> name = std::move(event->name);
  ...
  js_call_function(env, function, js_receiver_t(receiver), js_object_t(peripheral_ext), id, name, rssi, js_object_t(service_data));
...
static void
bare_bluetooth_apple_central__teardown(BareBluetoothAppleCentral *central) {
  ...
  dispatch_async(central->queue, ^{
    uv_async_send(central->cleanup_async);
  });
}

static void
bare_bluetooth_apple_central__on_teardown(js_deferred_teardown_t *handle, void *data) {
  bare_bluetooth_apple_central__teardown((__bridge BareBluetoothAppleCentral *) data);
}
...
bare_bluetooth_apple_central_init(
  ...
  @autoreleasepool {
    ...
    err = js_add_deferred_teardown_callback(env, bare_bluetooth_apple_central__on_teardown, (__bridge void *) handle, &handle->teardown);
    assert(err == 0);

    auto *state_ctx = new bare_bluetooth_apple_central_t{(__bridge BareBluetoothAppleCentral *) CFBridgingRetain(handle)};
    err = js_create_threadsafe_function<
      bare_bluetooth_apple_central__on_state_change,
      bare_bluetooth_apple__on_finalize<bare_bluetooth_apple_central_t>,
    ...
    handle->cleanup_async = new uv_async_t;
    err = uv_async_init(loop, handle->cleanup_async, bare_bluetooth_apple_central__on_cleanup);
    assert(err == 0);
...
bare_bluetooth_apple_central_connect(
  ...
  @autoreleasepool {
    ...
    [central->manager connectPeripheral:wrapper->peripheral options:nil];
  }
}
...
bare_bluetooth_apple_central_destroy(
  ...
  @autoreleasepool {
    ...
    bare_bluetooth_apple_central__teardown(central);
  }
}
...
- (void)stream:(NSStream *)stream handleEvent:(NSStreamEvent)eventCode {
  ...
  case NSStreamEventErrorOccurred: {
    ...
    if (!event) abort();
    event->message = error ? error.localizedDescription.UTF8String : "Unknown stream error";
...
static void
bare_bluetooth_apple_l2cap__teardown(BareBluetoothAppleL2CAPChannel *l2cap, js_threadsafe_function_release_mode_t mode) {
  if (l2cap->finalized.exchange(true)) return;

  int err;

  err = js_release_threadsafe_function(l2cap->tsfn_open, mode);
  assert(err == 0);

  err = js_release_threadsafe_function(l2cap->tsfn_close, mode);
  assert(err == 0);

  err = js_release_threadsafe_function(l2cap->tsfn_error, mode);
  assert(err == 0);

  err = js_release_threadsafe_function(l2cap->tsfn_end, mode);
  assert(err == 0);

  err = js_release_threadsafe_function(l2cap->tsfn_drain, mode);
  assert(err == 0);

  err = js_release_threadsafe_function(l2cap->tsfn_data, mode);
  assert(err == 0);

  err = js_delete_reference(l2cap->env, l2cap->ctx);
  assert(err == 0);

  err = js_finish_deferred_teardown_callback(l2cap->teardown);
  assert(err == 0);
}
...
bare_bluetooth_apple_l2cap__on_close(
  ...
  bare_bluetooth_apple_l2cap__teardown(channel, js_threadsafe_function_release);
...
static void
bare_bluetooth_apple_l2cap__on_teardown(js_deferred_teardown_t *handle, void *data) {
  auto l2cap = (__bridge BareBluetoothAppleL2CAPChannel *) data;

  if (!l2cap->destroyed.load()) {
    [l2cap destroy];
  }

  bare_bluetooth_apple_l2cap__teardown(l2cap, js_threadsafe_function_abort);
}
...
bare_bluetooth_apple_l2cap_init(
  ...
  @autoreleasepool {
    ...
    err = js_create_threadsafe_function<
      bare_bluetooth_apple_l2cap__on_data,
      bare_bluetooth_apple__on_finalize<bare_bluetooth_apple_l2cap_t>,
    ...
    err = js_add_deferred_teardown_callback(env, bare_bluetooth_apple_l2cap__on_teardown, (__bridge void *) handle, &handle->teardown);
    assert(err == 0);
